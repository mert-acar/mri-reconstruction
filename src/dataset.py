...
class OCMRDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        data = pad(data, [(256 - data.shape[dim]) / 2 for dim in range(1,data.ndim)])
        data_und = pad(data_und, [(256 - data_und.shape[dim]) / 2 for dim in range(1,data_und.ndim)])
        k_und = pad(k_und, [(256 - k_und.shape[dim]) / 2 for dim in range(1,k_und.ndim)])
        mask = pad(mask, [(256 - mask.shape[dim]) / 2 for dim in range(1,mask.ndim)])
